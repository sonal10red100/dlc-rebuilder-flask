...
# ==============================================================================
#Line. No. 1260 ======> return _coordinates function
...
def return_coordinates(
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      if keypoints is not None:
        box_to_keypoints_map[box].extend(keypoints[i])
      if scores is None:
        print('')#box_to_color_map[box] = groundtruth_box_visualization_color
      else:
        display_str = ''
        if not skip_labels:
          if not agnostic_mode:
            if classes[i] in category_index.keys():
              ...
              class_name = 'N/A'
            display_str = str(class_name)
            box_to_color_map[box]=display_str
        if not skip_scores:
          if not display_str:
  ...
  for box, color in box_to_color_map.items():
    ...
    xmin = int(xmin*width)
    xmax = int(xmax*width)
    coordinates_list.append([ymin, ymax, xmin, xmax, (box_to_score_map[box]),(box_to_color_map[box])])
